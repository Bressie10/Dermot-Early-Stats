<script>
  ...
  async function startLive() {
    ...
    const { data, error } = await supabase
      .from('live_sessions')
      .insert({ host_user_id: $user.id, match_data: getLivePayload() })
      .select().single()
  ...
  function getLivePayload() {
    return { teamName: $settingsStore.teamName, opposition, period, score, stats, events, players }
  }
...
<div class="screen">
  ...
  <div class="live-share-row">
    {#if isLive}
      <div class="live-active-badge"><span class="live-pulse"></span> Live — others can watch in real time</div>
    ...
    {:else}
      ...
      <span class="live-hint">Let others watch this match in real time</span>
    {/if}
...
<style>
  ...
  .live-error { font-size: 12px; color: #c62828; width: 100%; }
